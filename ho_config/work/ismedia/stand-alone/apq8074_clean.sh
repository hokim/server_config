...
if [ -f build_clean.log.old ]; then
	rm -f build_clean.log.old
fi
if [ -f build_clean.log ]; then
	mv build_clean.log build_clean.log.old
fi

make clean -j4 BUILD_ID=APQ8074_$ITCVER 2>&1 | tee -a build_clean.log
